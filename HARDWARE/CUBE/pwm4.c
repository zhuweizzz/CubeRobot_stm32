...
void PWM4_GPIO_Init()
{
	...
	PWM4_GPIO_CLK_Cmd(PWM4_GPIO_CLK, ENABLE);
	
	GPIO_PinAFConfig(PWM4_GPIO, PWM4_GPIOSource, PWM4_GPIO_AF);
...
void PWM4_TIM_Init()
{
	...
	PWM4_TIM_CLK_Cmd(PWM4_TIM_CLK, ENABLE);
	...
	TIM_TimeBaseInit(PWM4_TIM, &TIM_TimeBaseInitStructure);
...
{
	TIM_SetCompare1(PWM4_TIM, scale);
}
